    ...
    query: {
      ...
      cashbackConfigurationTier: {
        async $allOperations({ operation, args, query }) {
          const before = isWrite(operation)
            ? await parentCashbackId(prisma, args)
            : null;
          const result = await query(args);
          if (!isWrite(operation)) return result;
      ...
      exclusiveOfferFeedTimeline: {
        async $allOperations({ operation, args, query }) {
          const before = isWrite(operation)
            ? await parentExclusiveId(prisma, args)
            : null;
          const result = await query(args);
          if (!isWrite(operation)) return result;
          const ids = new Set<string>();
      ...
      loyaltyTier: {
        async $allOperations({ operation, args, query }) {
          const before = isWrite(operation)
            ? await parentLoyaltyProgramId(prisma, args, "tier")
            : null;
          const result = await query(args);
          if (!isWrite(operation)) return result;
          const ids = new Set<string>();
      ...
      merchantLoyaltyReward: {
        async $allOperations({ operation, args, query }) {
          const before = isWrite(operation)
            ? await parentLoyaltyProgramId(prisma, args, "reward")
            : null;
          const result = await query(args);
          if (!isWrite(operation)) return result;
          const ids = new Set<string>();
      ...
      review: {
        async $allOperations({ operation, args, query }) {
          ...
          const [c, e, l, t, lt, mlr] = await Promise.all([
            prisma.cashbackConfiguration.findUnique({
              where: { reviewId: id },
              select: { id: true },
            }),
            prisma.exclusiveOffer.findUnique({
              where: { reviewId: id },
              select: { id: true },
            }),
            ...
            prisma.cashbackConfigurationTier.findUnique({
              where: { reviewId: id },
              select: { cashbackConfigurationId: true },
            }),
            prisma.loyaltyTier.findUnique({
              where: { reviewId: id },
              select: { loyaltyProgramId: true },
            }),
            prisma.merchantLoyaltyReward.findUnique({
              where: { reviewId: id },
              select: { loyaltyProgramId: true },
            }),
          ]);
...
async function parentCashbackId(
  prisma: PrismaClient,
  args: unknown,
): Promise<string | null> {
...
async function parentExclusiveId(
  prisma: PrismaClient,
  args: unknown,
): Promise<string | null> {
  const id = extractWhereId(args);
